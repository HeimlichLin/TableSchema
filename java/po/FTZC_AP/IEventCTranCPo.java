package com.doc.common.po; 
 
public interface IEventCTranCPo {
 
	String getPostspecialaccount();
 
	void setPostspecialaccount(final String postspecialaccount);
 
	String getPosttype();
 
	void setPosttype(final String posttype);
 
	String getType();
 
	void setType(final String type);
 
	String getYear();
 
	void setYear(final String year);
 
	String getCountrycode();
 
	void setCountrycode(final String countrycode);
 
	String getExchangeagency();
 
	void setExchangeagency(final String exchangeagency);
 
	BigDecimal getTotalpackageno();
 
	void setTotalpackageno(final BigDecimal totalpackageno);
 
	BigDecimal getBagno();
 
	void setBagno(final BigDecimal bagno);
 
	BigDecimal getPageno();
 
	void setPageno(final BigDecimal pageno);
 
	BigDecimal getCellno();
 
	void setCellno(final BigDecimal cellno);
 
	String getPostno();
 
	void setPostno(final String postno);
 
	BigDecimal getWeight();
 
	void setWeight(final BigDecimal weight);
 
	String getSealdatetime();
 
	void setSealdatetime(final String sealdatetime);
 
	String getProvincetype();
 
	void setProvincetype(final String provincetype);
 
	String getBoxtype();
 
	void setBoxtype(final String boxtype);
 
	String getFilename();
 
	void setFilename(final String filename);
 
}
