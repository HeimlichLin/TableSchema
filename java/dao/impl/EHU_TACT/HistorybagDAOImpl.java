...
public class HistorybagDAOImpl extends GeneralDAOImpl<HistorybagPo> implements HistorybagDAO { 
	...
	protected static final MapConverter<HistorybagPo> CONVERTER = new MapConverter<HistorybagPo>() { 
		...
		public HistorybagPo convert(final DataObject dataObject) { 
			...
			historybagPo.setChargeuser(dataObject.getString(HistorybagPo.COLUMNS.CHARGEUSER.name())); 
			historybagPo.setExpressfee(BigDecimalUtils.formObj(dataObject.getValue(HistorybagPo.COLUMNS.EXPRESSFEE.name()))); 
			historybagPo.setAirlinefee(BigDecimalUtils.formObj(dataObject.getValue(HistorybagPo.COLUMNS.AIRLINEFEE.name()))); 
			historybagPo.setFlightno(dataObject.getString(HistorybagPo.COLUMNS.FLIGHTNO.name())); 
			historybagPo.setLastchargedate(TimestampUtils.of(dataObject.getValue(HistorybagPo.COLUMNS.LASTCHARGEDATE.name()))); 
			historybagPo.setWorkarea(dataObject.getString(HistorybagPo.COLUMNS.WORKAREA.name())); 
			return historybagPo; 
		} 
		...
		public DataObject toDataObject(final HistorybagPo historybagPo) { 
			...
			dataObject.setValue(HistorybagPo.COLUMNS.CHARGEUSER.name(), historybagPo.getChargeuser()); 
			dataObject.setValue(HistorybagPo.COLUMNS.EXPRESSFEE.name(), historybagPo.getExpressfee()); 
			dataObject.setValue(HistorybagPo.COLUMNS.AIRLINEFEE.name(), historybagPo.getAirlinefee()); 
			dataObject.setValue(HistorybagPo.COLUMNS.FLIGHTNO.name(), historybagPo.getFlightno()); 
			dataObject.setValue(HistorybagPo.COLUMNS.LASTCHARGEDATE.name(), historybagPo.getLastchargedate()); 
			dataObject.setValue(HistorybagPo.COLUMNS.WORKAREA.name(), historybagPo.getWorkarea()); 
			return dataObject; 
		} 
