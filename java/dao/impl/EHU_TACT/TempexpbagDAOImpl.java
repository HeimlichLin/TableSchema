...
public class TempexpbagDAOImpl extends GeneralDAOImpl<TempexpbagPo> implements TempexpbagDAO { 
	...
	protected static final MapConverter<TempexpbagPo> CONVERTER = new MapConverter<TempexpbagPo>() { 
		...
		public TempexpbagPo convert(final DataObject dataObject) { 
			...
			tempexpbagPo.setChargeuser(dataObject.getString(TempexpbagPo.COLUMNS.CHARGEUSER.name())); 
			tempexpbagPo.setExpressfee(BigDecimalUtils.formObj(dataObject.getValue(TempexpbagPo.COLUMNS.EXPRESSFEE.name()))); 
			tempexpbagPo.setAirlinefee(BigDecimalUtils.formObj(dataObject.getValue(TempexpbagPo.COLUMNS.AIRLINEFEE.name()))); 
			tempexpbagPo.setFlightno(dataObject.getString(TempexpbagPo.COLUMNS.FLIGHTNO.name())); 
			tempexpbagPo.setLastchargedate(TimestampUtils.of(dataObject.getValue(TempexpbagPo.COLUMNS.LASTCHARGEDATE.name()))); 
			tempexpbagPo.setWorkarea(dataObject.getString(TempexpbagPo.COLUMNS.WORKAREA.name())); 
			return tempexpbagPo; 
		} 
		...
		public DataObject toDataObject(final TempexpbagPo tempexpbagPo) { 
			...
			dataObject.setValue(TempexpbagPo.COLUMNS.CHARGEUSER.name(), tempexpbagPo.getChargeuser()); 
			dataObject.setValue(TempexpbagPo.COLUMNS.EXPRESSFEE.name(), tempexpbagPo.getExpressfee()); 
			dataObject.setValue(TempexpbagPo.COLUMNS.AIRLINEFEE.name(), tempexpbagPo.getAirlinefee()); 
			dataObject.setValue(TempexpbagPo.COLUMNS.FLIGHTNO.name(), tempexpbagPo.getFlightno()); 
			dataObject.setValue(TempexpbagPo.COLUMNS.LASTCHARGEDATE.name(), tempexpbagPo.getLastchargedate()); 
			dataObject.setValue(TempexpbagPo.COLUMNS.WORKAREA.name(), tempexpbagPo.getWorkarea()); 
			return dataObject; 
		} 
