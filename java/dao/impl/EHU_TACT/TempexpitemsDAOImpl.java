...
public class TempexpitemsDAOImpl extends GeneralDAOImpl<TempexpitemsPo> implements TempexpitemsDAO { 
	...
	protected static final MapConverter<TempexpitemsPo> CONVERTER = new MapConverter<TempexpitemsPo>() { 
		...
		public TempexpitemsPo convert(final DataObject dataObject) { 
			...
			tempexpitemsPo.setDeclno(dataObject.getString(TempexpitemsPo.COLUMNS.DECLNO.name())); 
			tempexpitemsPo.setFlightdest(dataObject.getString(TempexpitemsPo.COLUMNS.FLIGHTDEST.name())); 
			tempexpitemsPo.setWorkarea(dataObject.getString(TempexpitemsPo.COLUMNS.WORKAREA.name())); 
			tempexpitemsPo.setUldflag(TimestampUtils.of(dataObject.getValue(TempexpitemsPo.COLUMNS.ULDFLAG.name()))); 
			...
			tempexpitemsPo.setPkgbagno(dataObject.getString(TempexpitemsPo.COLUMNS.PKGBAGNO.name())); 
		...
		public DataObject toDataObject(final TempexpitemsPo tempexpitemsPo) { 
			...
			dataObject.setValue(TempexpitemsPo.COLUMNS.DECLNO.name(), tempexpitemsPo.getDeclno()); 
			dataObject.setValue(TempexpitemsPo.COLUMNS.FLIGHTDEST.name(), tempexpitemsPo.getFlightdest()); 
			dataObject.setValue(TempexpitemsPo.COLUMNS.WORKAREA.name(), tempexpitemsPo.getWorkarea()); 
			dataObject.setValue(TempexpitemsPo.COLUMNS.ULDFLAG.name(), tempexpitemsPo.getUldflag()); 
			...
			dataObject.setValue(TempexpitemsPo.COLUMNS.PKGBAGNO.name(), tempexpitemsPo.getPkgbagno()); 
			return dataObject; 
		} 
