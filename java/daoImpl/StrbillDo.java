...
public class StrbillDAO extends GeneralDAOImpl<StrbillDo> implements StrbillDAO { 
	...
	public SqlWhere getPkSqlWhere(StrbillDo po) { 
		sqlWhere.add(StrbillDo.COLUMNS.MSGFUN.name(), po.getMsgfun()); 
		sqlWhere.add(StrbillDo.COLUMNS.GDSTYPE.name(), po.getGdstype()); 
		sqlWhere.add(StrbillDo.COLUMNS.CTMCODE.name(), po.getCtmcode()); 
		sqlWhere.add(StrbillDo.COLUMNS.SUBCON.name(), po.getSubcon()); 
		sqlWhere.add(StrbillDo.COLUMNS.OTHERS.name(), po.getOthers()); 
	} 
