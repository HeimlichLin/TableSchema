...
public class ProviderServiceImpl implements ProviderService {
	...
	public ProviderServiceImpl() {
		if (this.list == null) {
			...
			this.list = AllTabCommentsDAO.INSTANCE.getAllTableName(this.session);
		}
	}
	...
	private class MyConverter implements Converter<SchemaDTO> {
		...
		public SchemaDTO convert(RowMap paramDataObject) {
			...
			dto.setTableName(paramDataObject.getString(
					SchemaCode.TABLE_NAME.name()));
			dto.setTableComments(paramDataObject.getString(
					SchemaCode.TABLE_COMMENTS.name()));
			dto.setColumnName(paramDataObject.getString(
					SchemaCode.COLUMN_NAME.name()));
			dto.setColumnComments(paramDataObject.getString(
					SchemaCode.COLUMN_COMMENTS.name()));
			dto.setDataType(paramDataObject.getString(
					SchemaCode.DATA_TYPE.name()));
			dto.setDataSize(paramDataObject.getString(
					SchemaCode.DATA_SIZE.name()));
			dto.setNullAble(paramDataObject.getString(
					SchemaCode.NULLABLE.name()));
			dto.setPosition(paramDataObject.getString(
					SchemaCode.POSITION.name()));
			return dto;
		}
	...
	public void createTableSchema() {		
		List<List<SchemaDTO>> lists = this.getAllSchemaDTOs();
	...
	public void createJavePage() {
		for (AllTabCommentsDo vo : this.list) {
			this.logger.info("tableName : " + vo.getTableName());
			TableDTO tableDTO = new TableDTO();
			TableNameDTO tableNameDTO = new TableNameDTO(vo.getTableName());
			List<SchemaDTO> list = this.getSchemaDTOList(vo.getTableName());
	...
	public void createJavePage(String tableName) {
		...
		TableDTO tableDTO = new TableDTO();
		TableNameDTO tableNameDTO = new TableNameDTO(tableName);
		List<SchemaDTO> list = this.getSchemaDTOList(tableName);
		tableDTO.setDto(tableNameDTO);
		tableDTO.setList(list);
		JavaPage javaPage = new JavaPage();
		javaPage.createJavaPageDo(tableDTO);
	...
	private List<List<SchemaDTO>> getAllSchemaDTOs() {
		List<List<SchemaDTO>> lists = new ArrayList<List<SchemaDTO>>();
		for (AllTabCommentsDo vo : this.list) {
			this.logger.info("tableName : " + vo.getTableName());
	...
	private List<SchemaDTO> getSchemaDTOList(String tableName){
		...
		return this.session.select(new MyConverter(), ColumDetailSql.getSQL(), sqlWhere);
	}
